...
export const convertIndexToDayKor = (dateIndex: Number) => {
	let kor
	switch (dateIndex) {
    case 0:{
      kor = "월"
...
export const convIntToHr = (integer: number) => {
	let _hour
...
export const statusToKor = (status: String) => {
  let kor = "";
  switch (status) {
    case "recruiting":{
      kor = "모집 중인 스터디"
...
export const convertMoneyUnitKor = (integer: Number) => {
  if(!integer){
    return null
  }
